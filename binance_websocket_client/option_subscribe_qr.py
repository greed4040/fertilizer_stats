...
def subscribe_to_contracts(ws):
    ...
    global contracts_date_buffer
    global request_ctr
    request_ctr += 1

    contracts = uh2.get_all_filtered_read_only()

    contract_names_only = []
    for collection in contracts:
        current_collection = contracts[collection]
        for el in current_collection:
            contract_names_only.append(el['symbol'])
    print(contract_names_only)

    params = {
        "method": "SUBSCRIBE",
        "params": [f"{contract}@depth10" for contract in contract_names_only],
        "id": request_ctr
    }
    time.sleep(5)
...
def periodic_check(ws):
    ...
    while True:
        ...
        global contracts_date_buffer
        current_option_contracts_date_from_db = uh2.get_current_expiry_date_from_redis()

        if contracts_date_buffer != current_option_contracts_date_from_db:
            contracts_date_buffer = current_option_contracts_date_from_db 
        ...
        time.sleep(1)  # Wait for 1 minute before the next check
